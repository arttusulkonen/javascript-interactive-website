...
window.addEventListener('DOMContentLoaded', function () {
  // создаём пустой объект, куда будем сохранять данные из модульного окна
  var modalState = {};
  Object(_modules_changeModalState__WEBPACK_IMPORTED_MODULE_4__["default"])(modalState);
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])();
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.glazing_slider', '.glazing_block', '.glazing_content', 'active');
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.decoration_slider', '.no_click', '.decoration_content > div > div', 'after_click'); ///только прямые наследники с тегом img
        ...
        switch (item.nodeName) {
          ...
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              i === 0 ? state[prop] = 'Холодное' : state[prop] = 'Тёплое'; /// пользователь может поставить только одну галочку

              elem.forEach(function (box, j) {
                box.checked = false; // убирает все галочки

                if (i == j) {
                  // но если i == j, то ставит галочку
                  box.checked = true;
                }
              });
            } else {
...
var checkNumInputs = function checkNumInputs(selector) {
  var numInputs = document.querySelectorAll(selector); // в numImputs передаём селектор нужной формы, куда воодятся данные

  numInputs.forEach(function (item) {
    item.addEventListener('input', function () {
      item.value = item.value.replace(/\D/, ''); // если пользователь вводит не число, то регулярное выражение это находит и заменяет пустой строкой
    });
  });
};
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    ...
    item.addEventListener('submit', function (e) {
      ...
      item.appendChild(statusMessage); // item кладём в FormData. этим методом собираем все данные из инпута и кладём в formData

      var formData = new FormData(item); // ниже проверяем, если мы дошли уже до финальной формы, то берём данные из state, затем их перебираем
...
var modals = function modals() {
  // создаём общую функцию. 1)получаем селлектор 2)какое именно окно 3)кнопка закрытия 4)при нажатии на подложку, окно закрывается
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        document.body.style.overflow = 'hidden'; // отменяет прокрутку страницы при открытом диалоговом окне
      });
    });
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    content[i].style.display = display; //т.к получаем псевдомассив, то по индексу элемента получаем нужный элемент

    tab[i].classList.add(activeClass);
  }

  hideTabContent();
  showTabContent(); // вешаем обработчик событий на область, которая совмещает в себе все табы

  header.addEventListener('click', function (e) {
    var target = e.target; // в таргет сохраняем элемент, где произошло событие
    ...
    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      ...
      tab.forEach(function (item, i) {
        if (target == item || target.parentNode == item) {
          // если кликнутый элемент == элементу, который сейчас перебирается
          hideTabContent();
          showTabContent(i);
        }
      });
